...
class _TournamentsPageState extends State<TournamentsPage> {
  ...
  Future<void> getTournaments() async {
    try {
      ...
      setState(() {
        for (var tournament in json.decode(response.body)["data"]) {
          Tournament t = Tournament.fromJson(tournament);
          if (t.seasonEnd.toLocal().isAfter(DateTime.now())) {
                  ...
                  GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return Card(
                        child: InkWell(
                          onTap: () {
                            router.navigateTo(context, "/tournaments/${pastTournaments[index].id}", transition: TransitionType.fadeIn);
                          },
                          child: SizedBox(
                            height: 300,
                            child: ClipRRect(
                              ...
                              child: Stack(
                                ...
                                children: [
                                  SizedBox(
                                    ...
                                    width: MediaQuery.of(context).size.width,
                                    child: ExtendedImage.network(
                                      pastTournaments[index].bannerURL,
                                      fit: BoxFit.cover,
                                      width: double.infinity,
                                    ),
                                  ),
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Row(
                                          ...
                                          children: [
                                            Visibility(
                                              visible: currentTournaments.any((element) => element.id == pastTournaments[index].id),
                                              child: Card(
                                                color: Theme.of(context).colorScheme.background,
                                                child: Container(
                                                  padding: const EdgeInsets.only(left: 16, right: 16, top: 8, bottom: 8),
                                          ...
                                          children: [
                                            Expanded(
                                              child: Column(
                                                ...
                                                crossAxisAlignment: CrossAxisAlignment.start,
                                                children: [
                                                  Text(pastTournaments[index].name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 32, color: Colors.white)),
                                                  Text("${DateFormat("MMM d").format(pastTournaments[index].seasonStart)} - ${DateFormat("MMM d, yyyy").format(pastTournaments[index].seasonEnd)}", style: const TextStyle(fontSize: 24, color: Colors.grey)),
                                                ],
                                              ),
                                            ),
                                            const Padding(padding: EdgeInsets.all(8)),
                                            Image.asset(getGameImage(pastTournaments[index].game), width: 55, height: 55),
                                          ],
                                        ),
                                      ],
                                    ),
